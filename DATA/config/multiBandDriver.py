...
import lsst.meas.extensions.astrometryNet.multiindex
...
import eups.Product
import lsst.meas.extensions.astrometryNet.loadAstrometryNetObjects
import lsst.meas.extensions.astrometryNet.astrometry_net.astrometry_net
...
import lsst.meas.extensions.astrometryNet.astrometryNetDataConfig
import eups.stack
import eups.hooks
import lsst.shapelet.functorKeys
import lsst.meas.extensions.astrometryNet
import eups.distrib.eupspkg
import lsst.meas.extensions.astrometryNet.version
import eups
import eups.distrib.DistribFactory
import eups.table
import lsst.meas.extensions.astrometryNet.astrometry_net.astrometry_netContinued
import lsst.shapelet.constants.constantsContinued
import lsst.shapelet.matrixBuilder
import lsst.meas.extensions.astrometryNet.anetBasicAstrometry
import lsst.meas.extensions.astrometryNet.anetAstrometry
import eups.app
import lsst.meas.modelfit.adaptiveImportanceSampler
import lsst.shapelet.radialProfile
import eups.distrib.builder
import lsst.meas.extensions.astrometryNet.astrometry_net
...
config.measureCoaddSources.deblend.weightTemplates=False

# Try to remove similar templates?
config.measureCoaddSources.deblend.removeDegenerateTemplates=False
...
config.measureCoaddSources.deblend.maxTempDotProd=0.5

# Apply a smoothing filter to all of the template images
config.measureCoaddSources.deblend.medianSmoothTemplate=True
...
# If the maximum of the gradient falls below this threshold, consider the algorithm converged
config.measureCoaddSources.measurement.plugins['modelfit_CModel'].initial.optimizer.gradientThreshold=0.01
...
config.measureCoaddSources.measurement.plugins.names=['base_PixelFlags', 'ext_convolved_ConvolvedFlux', 'ext_shapeHSM_HsmShapeRegauss', 'base_SkyCoord', 'base_PsfFlux', 'base_LocalBackground', 'ext_shapeHSM_HsmSourceMoments', 'ext_shapeHSM_HsmPsfMoments', 'base_SdssShape', 'base_GaussianFlux', 'base_SdssCentroid', 'base_NaiveCentroid', 'base_InputCount', 'ext_shapeHSM_HsmSourceMomentsRound', 'subaru_FilterFraction', 'base_Variance', 'modelfit_CModel', 'base_CircularApertureFlux', 'base_Blendedness', 'ext_photometryKron_KronFlux', 'modelfit_DoubleShapeletPsfApprox']
# whether to run this plugin in single-object mode
...
# If the maximum of the gradient falls below this threshold, consider the algorithm converged
config.measureCoaddSources.measurement.undeblended['modelfit_CModel'].initial.optimizer.gradientThreshold=0.01
...
# Select objects with value less than this
config.measureCoaddSources.match.sourceSelection.unresolved.maximum=None
...
config.measureCoaddSources.match.sourceSelection.signalToNoise.errField='base_PsfFlux_fluxSigma'
...
import lsst.meas.extensions.astrometryNet.loadAstrometryNetObjects
import lsst.meas.algorithms.loadReferenceObjects
config.measureCoaddSources.match.refObjLoader.retarget(target=lsst.meas.extensions.astrometryNet.loadAstrometryNetObjects.LoadAstrometryNetObjectsTask, ConfigClass=lsst.meas.algorithms.loadReferenceObjects.LoadReferenceObjectsConfig)
...
config.measureCoaddSources.catalogCalculation.plugins.names=['base_FootprintArea', 'base_ClassificationExtendedness']
# Priority-ordered list of bands for the merge.
...
# If the maximum of the gradient falls below this threshold, consider the algorithm converged
config.forcedPhotCoadd.measurement.plugins['modelfit_CModel'].initial.optimizer.gradientThreshold=0.01
...
config.forcedPhotCoadd.measurement.plugins.names=['base_PixelFlags', 'ext_convolved_ConvolvedFlux', 'base_TransformedShape', 'modelfit_CModel', 'base_CircularApertureFlux', 'base_TransformedCentroid', 'base_Variance', 'base_PsfFlux', 'base_LocalBackground', 'base_SdssShape', 'ext_photometryKron_KronFlux', 'base_GaussianFlux', 'base_SdssCentroid', 'modelfit_DoubleShapeletPsfApprox', 'base_InputCount']
# whether to run this plugin in single-object mode
...
# list of target seeings (FWHM, pixels)
config.forcedPhotCoadd.measurement.undeblended['ext_convolved_ConvolvedFlux'].seeing=[3.5, 5.0, 6.5]
...
# If the maximum of the gradient falls below this threshold, consider the algorithm converged
config.forcedPhotCoadd.measurement.undeblended['modelfit_CModel'].initial.optimizer.gradientThreshold=0.01
...
config.forcedPhotCoadd.measurement.undeblended.names=['base_PsfFlux', 'base_CircularApertureFlux', 'ext_photometryKron_KronFlux']
...
# flux measurement algorithms to be aperture-corrected by reference to another algorithm; this is a mapping alg1:alg2, where 'alg1' is the algorithm being corrected, and 'alg2' is the algorithm supplying the corrections
config.forcedPhotCoadd.applyApCorr.proxies={'undeblended_base_PsfFlux_flux': 'base_PsfFlux_flux', 'undeblended_ext_photometryKron_KronFlux_flux': 'ext_photometryKron_KronFlux_flux'}
